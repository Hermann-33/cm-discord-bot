...
export function reviewFirstTurnObservability(query, aliases) {
  ...
  const explicitOrderReference = has(/\b(?:order id|order identifier|order reference)\b|\[order identifier omitted\]/u);
  ...
  if (has(/\b(?:phishing|security report|malware|bot token|discords? team|cloudflare|vercel)\b/u) && has(/\b(?:report(?:ed|ing)?|reverse(?:d| engineered)?|token|proof|banned|phishing|rat|malware)\b/u)) {
    return {
      ...
      primaryDecision: 'human_escalation',
      observableCaseIds: [],
      observableFamilyIds: ['support.security'],
      clarificationId: null,
      decisionReason: 'The opening message is a security or abuse report that requires human review.'
    };
  }
  ...
  if (has(/\b(?:bypass|evad(?:e|ing|ion)|unban|anti.?cheat|anti cheat|failed to load driver|inject(?:ion)?)\b/u) || has(/\b(?:is|are|still|currently|rn)\b.{0,25}\b(?:undetected|detected|ud)\b|\b(?:undetected|detected|ud)\b.{0,20}\b(?:rn|right now|currently|status)\b/u)) {
    return { ...result, ...control('direct_restricted_escalation', ['case.restricted.technical'], 'The first turn explicitly enters the restricted technical or detection-status boundary.', { observableFamilyIds: ['restricted'] }) };
  }
  if (has(/\b(?:refund|cancel(?:lation)?|replacement|replace|warranty|wrong delivery|wrong account|charged twice|double charged)\b/u)) {
    ...
    return { ...result, ...control('direct_policy_route', cases, 'The customer explicitly requests or disputes a current-authority remedy.', { policyRoute: true, observableFamilyIds: ['commerce.policy'] }) };
  }
  if (has(/\b(?:customer role|link(?:ed|ing)? (?:my )?discord|discord (?:is )?linked|dont close (?:the )?ticket|do not close (?:the )?ticket|close (?:the )?ticket)\b/u)) {
    return { ...result, ...control('direct_support_operation', ['case.dashboard.verification','case.support.followup'], 'The opening request is an observable support-operation task.', { observableFamilyIds: ['support.operations'] }) };
  }
  if (has(/\[attachment omitted\]/u) && (!technicalSignal || has(/\b(?:this|that|what|why|issue|problem|error)\b/u))) {
    return { ...result, ...control('direct_attachment_route', ['case.attachment.review'], 'The issue depends on customer-provided visual evidence that requires attachment review.', { observableFamilyIds: ['support.attachment'] }) };
  }
  ...
  if (has(/\bhwid\b.{0,20}\breset\b|\breset\b.{0,20}\bhwid\b/u)) {
    return { ...result, ...exactCase('case.spoofer.hwid_state', 'The opening request explicitly asks about an HWID reset or state change.') };
  }
  if (nfaSignal && has(/\b(?:activate|activation|redeem|token|setup guide|guide|how (?:do i|to) use)\b/u)) {
    return { ...result, ...exactCase('case.nfa.redemption_activation', 'NFA identity and activation, token, guide, or redemption intent are explicit.') };
  }
  ...
  if (spooferSignal && has(/\b(?:perm(?:anent)?|temp(?:orary)?|duration|lifetime|how long)\b/u)) {
    return { ...result, ...exactCase('case.catalog.pricing_duration', 'The first turn explicitly asks about the spoofer duration or permanent/temporary offering.') };
  }
  ...
  if (has(/\b(?:in stock|out of stock|restock|available|status|working rn|up rn|price|how much|is .{0,30} working)\b/u) && (has(/\b(?:product|cheat|spoofer|account|nfa|rust|cs2|fortnite|apex|pubg|eft|warzone|r6|exodus|ancient|venom)\b/u) || entities.length > 0)) {
    ...
    return { ...result, ...control('direct_dynamic_lookup', cases, 'The customer asks for current catalog state, stock, status, or price.', { dynamicLookupIds: ['dynamic.catalog.product_status'], observableFamilyIds: families }) };
  }

  if (has(/\b(?:spoofer|spoof)\b/u) && has(/\b(?:put|restore|change).{0,25}\b(?:pc|computer|hwid|machine)\b.{0,20}\b(?:back|normal)|\b(?:revert|reverse|unspoof|remove)\b/u)) return { ...result, ...exactCase('case.spoofer.reversal_reset', 'The opening message explicitly asks to reverse or reset a temporary spoof state.') };
  ...
  if (spooferSignal && has(/\b(?:not working|doesnt work|dont work|wont work|problem|launch(?:ing|ed)?|bsod|crash(?:es|ing)?|error)\b/u)) {
