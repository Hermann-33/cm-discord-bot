...
const INTENT_RULES = [
  {
    ...
    patterns: [/didn.?t mean to (?:make|open|create)/i, /accident(?:al|ally)?(?: opened| made| created)?/i, /wrong ticket/i, /close (?:this|ticket)/i]
  },
  {
    id: 'order_not_received',
    label: 'Order Not Received',
    category: 'Orders & Delivery',
    patterns: [/not received/i, /didn.?t receive/i, /haven.?t received/i, /never received/i, /where(?:'s| is) my (?:order|key|license|account)/i, /nothing (?:was )?delivered/i, /order.*(?:missing|not here)/i]
  },
  {
    id: 'manual_license_delivery',
    label: 'Manual License Delivery',
    category: 'Orders & Delivery',
    patterns: [/generate my key/i, /key manually/i, /manual(?:ly)? generate/i, /send my order id/i, /here(?:'s| is) my order id/i, /license.*order id/i]
  },
  {
    id: 'license_invalid_or_expired',
    label: 'License Invalid Or Expired',
    category: 'Licensing',
    patterns: [/key (?:is )?invalid/i, /invalid (?:key|license)/i, /license (?:is )?invalid/i, /key (?:doesn.?t|does not|won.?t) work/i, /license (?:doesn.?t|does not|won.?t) work/i, /key expired/i, /license expired/i, /expired key/i]
  },
  {
    id: 'payment_issue',
    label: 'Payment Issue',
    category: 'Payments',
    patterns: [/payment (?:failed|error|issue|problem|pending|declined)/i, /card (?:declined|failed|charged)/i, /checkout (?:failed|error|issue)/i, /charged (?:twice|but|and)/i, /crypto (?:payment|transaction)/i, /payment.*(?:not going|won.?t|doesn.?t)/i]
  },
  {
    id: 'refund_request',
  ...
  },
  {
    ...
    patterns: [/customer role/i, /give me (?:my )?role/i, /purchase role/i, /verify (?:my )?purchase/i, /role.*(?:purchase|bought|order)/i, /access (?:to )?(?:the )?(?:customer|download|support) channel/i]
  },
  {
    id: 'restock_availability',
    label: 'Restock Or Availability',
    category: 'Product Availability',
    patterns: [/restock/i, /out of stock/i, /sold out/i, /when.*(?:back|available|stock)/i, /(?:is|are).*available/i, /stock.*(?:when|soon|eta)/i]
  },
  {
  ...
  },
  {
    ...
    category: 'Business',
    patterns: [/partnership/i, /partner with/i, /affiliate/i, /resell/i, /reseller/i, /media for/i, /content creator/i, /payment processing/i, /collab/i, /collaboration/i, /sponsor/i]
  },
  {
    id: 'download_or_documentation',
    label: 'Download Or Documentation',
    category: 'Product Access',
    patterns: [/where.*download/i, /download (?:link|page|file)/i, /how.*download/i, /docs/i, /documentation/i, /guide/i, /setup instructions/i, /install(?:ation)? (?:guide|instructions)/i]
  },
  {
    ...
    patterns: [/loader.*(?:error|fail|stuck|crash|close|open)/i, /(?:error|failed).*loader/i, /won.?t (?:launch|open|start)/i, /doesn.?t (?:launch|open|start)/i, /crash(?:es|ing)? (?:on|when|after) (?:launch|open|start)/i, /stuck (?:on|at).*(?:loader|loading)/i]
  },
  {
    id: 'account_credentials',
    label: 'Account Credentials Or Login',
    category: 'Accounts',
    patterns: [/wrong password/i, /password (?:doesn.?t|does not|won.?t) work/i, /can.?t log ?in/i, /cannot log ?in/i, /login (?:error|issue|problem)/i, /credentials/i, /email.*password/i, /account.*(?:locked|login|password|email)/i]
  },
  {
  ...
  },
  {
    ...
    category: 'Technical Support',
    restrictedTechnical: true,
    patterns: [/fps/i, /frame(?:s)? (?:drop|low)/i, /lag(?:ging|gy)?/i, /stutter/i, /game (?:crash|crashes|crashing|freeze|freezes)/i]
  },
  {
    id: 'update_or_service_outage',
    label: 'Update Or Service Outage',
    category: 'Service Status',
    patterns: [/outdated/i, /needs? (?:an )?update/i, /update (?:when|eta|issue|problem)/i, /maintenance/i, /service.*(?:down|offline)/i, /(?:loader|product|service) (?:is )?(?:down|offline)/i]
  },
  {
    id: 'replacement_request',
    label: 'Replacement Request',
    category: 'Orders & Delivery',
    patterns: [/replacement/i, /replace (?:it|this|my|the)/i, /need (?:a )?new (?:account|key|license)/i, /(?:account|key|license).*(?:dead|revoked)/i]
  },
  {
  ...
  },
  {
    ...
    category: 'Restricted Technical Support',
    restrictedTechnical: true,
    patterns: [/config(?:uration)?/i, /settings?/i, /aimbot/i, /\besp\b/i, /menu (?:option|setting|feature)/i, /feature.*(?:work|enable|disable)/i]
  }
];
...
export async function mineSupportKnowledge(options) {
  ...
  for (const record of review) {
    ...
    const matchText = normalizeForMatching(rawCustomer);
    const assignments = [];
    ...
    for (const rule of INTENT_RULES) {
      ...
      if (score > 0) {
        ...
        intentBuckets.get(rule.id).push({ record, score, customer: sanitizeExcerpt(rawCustomer) });
      }
    }
    ...
    ticketAssignments.push({
      transcriptId: record.transcriptId,
      intents: assignments,
      restrictedTechnical: assignments.some((assignment) => INTENT_RULES.find((rule) => rule.id === assignment.id)?.restrictedTechnical === true)
    });
  ...
  const intents = INTENT_RULES.map((rule) => {
    ...
    const samples = stableSamples(bucket, options.sampleLimit).map(({ record, score, customer }) => ({
      transcriptId: record.transcriptId,
      score,
      customer,
    ...
    for (const item of bucket) {
      for (const phrase of documentPhrases(customerText(item.record))) increment(phraseCounts, phrase);
    ...
    return {
      ...
      topCustomerPhrases: topEntries(phraseCounts, 25, 2),
  ...
  const outputManifest = {
    ...
    classifiedTicketCount: ticketAssignments.filter((entry) => entry.intents.length > 0).length,
  ...
  const readme = `# Support Knowledge Mining\n\n` +
    `Deterministic first-pass mining over all ${expectedCount} transcript review records.\n\n` +
    `- \`intent-candidates.json\` — candidate intent buckets with counts, phrases, recurring staff responses and evidence samples.\n` +
    `- \`ticket-intents.ndjson\` — per-ticket rule matches for coverage/review.\n` +
    `- \`phrases.json\` — corpus-wide customer phrases/tokens and recurring staff response patterns.\n` +
    `- \`unclassified.json\` — representative tickets that matched no current intent rule.\n` +
  ...
  await writeFile(join(options.outputDir, 'intent-candidates.json'), `${JSON.stringify({ schemaVersion: 1, intents }, null, 2)}\n`);
  ...
  return {
    ...
    classifiedTicketCount: outputManifest.classifiedTicketCount,
    unclassifiedTicketCount: outputManifest.unclassifiedTicketCount,
...
async function main() {
  ...
  console.log(`[transcripts] support mining complete: transcripts=${result.transcriptCount} classified=${result.classifiedTicketCount} unclassified=${result.unclassifiedTicketCount}`);
