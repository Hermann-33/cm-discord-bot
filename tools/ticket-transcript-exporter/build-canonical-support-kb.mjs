...
async function writeMarkdownGraph(canonicalDir, entities, cases, nodes, policies, contradictions, requirements, compatibility) {
  ...
  const note = async (dir, id, title, links = []) => writeFile(join(canonicalDir, dir, `${id}.md`), `---\nid: ${id}\n---\n\n# ${title}\n\n${links.map((link) => `- [[${link}]]`).join('\n')}\n`);
  ...
  for (const item of cases) await note('Cases', item.id, item.displayName, unique([...item.recognition.symptomIds, ...item.possibleCauseIds, ...item.diagnosticIds, ...item.resolutionFlow.map((flow) => flow.procedureId), ...item.policyIds, ...item.relatedCaseIds]));
...
export async function buildCanonicalSupportKb(dataDir) {
  ...
  await writeJson(join(runtimeDir, 'manifest.json'), { schemaVersion: 1, knowledgeVersion: VERSION, generatedAt: new Date().toISOString(), sourceDomainSnapshotAt: snapshot.capturedAt, counts: { entities: entities.length, products: snapshot.products.length, variants: entities.filter((item) => item.type === 'variant').length, cases: runtimeCases.length, procedures: nodes.procedures.length, policies: policies.length, aliases: aliases.length, dynamicLookups: dynamicLookups.length }, retrieval: { method: 'exact-alias+scope-filter+bm25-style', topK: 5 }, evaluation: { datasetVersion: VERSION, queryCount: queries.length }, privacy: { rawTranscriptProse: false, customerPii: false }, runtimeBytes });
