...
export function reviewFirstTurnObservability(query: string, aliases: readonly RuntimeAliasRecord[]): FirstTurnDecision {
  ...
  if (has(/\b(?:do|make|create|be|become|apply for|looking for|looking to do|need)\s+(?:some\s+)?media\b|\bmedia\s+(?:creator|application)\b|\bmedia\b.{0,24}\b(?:tiktok|youtube)\b/u)) return withBase(result, exactCase("case.media.application", "The first turn explicitly asks about media or creator work."));
  ...
  if (has(/\b(?:where(?: is|'s)? (?:the )?(?:config(?:uration)? file|guide|setup guide)|how (?:do i|to) (?:start|set up|setup|install|configure)|defender is (?:on|off)|windows defender)\b/u)) return withBase(result, exactCase("case.product.requirements", "The opening message explicitly asks for setup, guide, configuration, or prerequisite help."));
  ...
  if (nfaSignal && has(/\b(?:someone else|owner.*(?:online|active|playing|joined|kicked)|owner joined|kicked me out|keeps logging|logged me out|sign(?:ed)? me out|asking for (?:a )?password|password (?:changed|required)|owner.{0,30}(?:no access|lost access))\b/u)) return withBase(result, exactCase("case.nfa.owner_session_conflict", "The first turn explicitly identifies NFA owner/session conflict behavior."));
  if (nfaSignal && has(/\b(?:worked (?:before|yesterday|earlier)|used to work|stopped working|became invalid|invalid after|no longer works|later invalid)\b/u)) return withBase(result, exactCase("case.nfa.invalid_after_use", "The first turn states that the NFA worked before and later became invalid."));
