...
export function reviewFirstTurnObservability(query: string, aliases: readonly RuntimeAliasRecord[]): FirstTurnDecision {
  ...
  if (paymentSignal && (explicitPaymentMethodPurchase || has(/\b(?:pending|processing|under review|checking|charged|paid|completed|declined|disabled|unavailable|locked|processor down|failed|wont work|doesnt work|didnt go through|doesnt go through|did not go through|did not arrive|nothing (?:appeared|arrived)|not credited|can i (?:buy|pay)|isnt detecting|not detecting|not detected)\b/u))) {
    const cases: string[] = [];
    ...
    if (has(/\b(?:pending|processing|under review|checking|failed|didnt go through|doesnt go through|did not go through|did not arrive|wont work|doesnt work|isnt detecting|not detecting|not detected)\b/u)) cases.push("case.payment.failed_or_pending");
  ...
  if (has(/\b(?:banned|game ban(?:ned)?|vac ban(?:ned)?|cooldown|limited matchmaking)\b/u)) return withBase(result, control("direct_policy_route", ["case.account.banned"], "The opening message explicitly reports an account enforcement state requiring current policy handling.", { observableFamilyIds: ["commerce.policy"] }));
