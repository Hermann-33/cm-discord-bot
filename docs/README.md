...
6. `decisions/ADR-0019-nonfatal-leaderboard-startup.md` — leaderboard failures cannot terminate the whole Discord runtime.
7. `decisions/ADR-0018-manual-pending-purchase-approval.md` — guarded manual approval for verified pending payments plus direct-result sharing.
8. `decisions/ADR-0017-direct-admin-mutation-slash-commands.md` — direct `/cm aura`, `/cm balance`, and `/cm refund` mutation model.
9. `decisions/ADR-0016-tickety-account-link-gate.md` — current Tickety account-link authorization/persistence/override model.
10. `decisions/ADR-0015-single-channel-visible-ai-test.md` — current narrow live-test authorization.
11. `context/ROADMAP.md` — current project phases and release gates.
12. `context/ARCHITECTURE.md` — accepted architecture/invariants.
13. `context/DATA_STATUS.md` — backend/data dependency details; use current-state docs for deployment-status supersession.
14. `context/CODEBASE_MAP.md` — module ownership and fragile boundaries.
15. `context/COMMANDS.md` — command policy.
16. `context/WORKFLOW.md` — task/audit/Git lifecycle.
17. `context/AUDIT_LOG.md` and `context/PROJECT_HISTORY.md` — chronology.
...
- ADR-0019 — leaderboard startup/failure is isolated from Discord process liveness
