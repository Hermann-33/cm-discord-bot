...
async function main() {
  ...
  if (manifest.status !== "frozen_for_deterministic_preflight" || manifest.fixtureSha256 !== fixtureSha256) throw new Error("B0-v4 frozen fixture mismatch");
  const fixture = JSON.parse(fixtureRaw);
  if (fixture.schemaVersion !== 1 || fixture.rows.length !== manifest.rows) throw new Error("Invalid B0-v4 fixture/manifest");
  ...
  const results = fixture.rows.map((row) => {
    ...
    const schema = buildSupportTriageJsonSchema(context.input);
    const reasons = [];
    ...
    if (!actionMatches(row.expected, fallback)) reasons.push(`fallback:${fallback.nextAction}`);
    if (!validation.valid) reasons.push(`fallback_validation:${validation.errors.join(",")}`);
    const actionEnum = schema?.properties?.nextAction?.enum;
  ...
  writeFileSync(resolve(root, "release-validation/results/b0-v4-preflight-result.json"), `${JSON.stringify(summary, null, 2)}\n`, "utf8");
  process.stdout.write(`${JSON.stringify(summary, null, 2)}\n`);
  if (failures.length) process.exitCode = 1;
}

main().catch((error) => {
  process.stderr.write(`${error instanceof Error ? error.message : "Unknown B0-v4 preflight failure"}\n`);
  process.exitCode = 1;
});
