...
function matches(expected, decision) {
...
async function main() {
  ...
  if (manifest.status !== "frozen_for_deterministic_preflight" || fixtureSha256 !== manifest.fixtureSha256) throw new Error("B0-v6 frozen fixture mismatch");
  ...
  if (runtime.knowledgeVersion !== manifest.runtimeKnowledgeVersion) throw new Error("B0-v6 runtime mismatch");
  ...
  const results = fixture.rows.map((row) => {
    ...
    if (!matches(row.expected, fallback)) reasons.push(`fallback:${fallback.nextAction}:${fallback.clarificationId ?? "none"}`);
    ...
    if (row.expected.action === "answer_case" && !any(row.expected.caseIds, context.input.allowed.caseIds)) reasons.push(`allowed_case:${context.input.allowed.caseIds.join(",") || "none"}`);
    ...
    if (row.expected.action === "request_dynamic_lookup" && !all(row.expected.lookupIds, context.input.allowed.dynamicLookupIds)) reasons.push(`allowed_lookups:${context.input.allowed.dynamicLookupIds.join(",") || "none"}`);
    if (row.expected.action === "request_policy_route" && !all(row.expected.policyIds, context.input.allowed.deterministicPolicyIds ?? [])) reasons.push(`deterministic_policies:${(context.input.allowed.deterministicPolicyIds ?? []).join(",") || "none"}`);
  ...
  const summary = {schemaVersion:1,evaluationClass:manifest.classification,fixture:fixture.name,fixtureSha256,productionCandidateSha:manifest.productionCandidateSha,privateCorpusSha:manifest.privateCorpusSha,runtimeKnowledgeVersion:runtime.knowledgeVersion,rows:results.length,passed:results.length-failures.length,failed:failures.length,passRate:(results.length-failures.length)/results.length,failures};
  mkdirSync(resolve(root,"release-validation/results"),{recursive:true});
  writeFileSync(resolve(root,"release-validation/results/b0-v6-preflight-result.json"),`${JSON.stringify(summary,null,2)}\n`);
  process.stdout.write(`${JSON.stringify(summary,null,2)}\n`);
  if (failures.length) process.exitCode=1;
}
main().catch((error)=>{process.stderr.write(`${error instanceof Error?error.message:"Unknown B0-v6 preflight failure"}\n`);process.exitCode=1;});
