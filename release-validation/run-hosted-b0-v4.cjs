...
const EXPECTED_CANDIDATE = "fb866a665260427944a4335db76e27ab17b3ec0f";
const EXPECTED_CORPUS = "c9e993f17583a607402f4173296f64aac52d2ebe";
const EXPECTED_RUNTIME = "1.0.0";
const EXPECTED_FIXTURE_SHA256 = "5a11e71e78f1fd0af06784e04f5f717304264f862d69e02109018665d66f27e4";
...
function estimatedTokens(value, maxCompletionTokens) {
  return Math.max(1, Math.ceil(JSON.stringify(value).length / 4) + maxCompletionTokens);
}

async function main() {
  const manifest = JSON.parse(readFileSync(resolve(root, "release-validation/manifest-b0-v4.json"), "utf8"));
  if (manifest.schemaVersion !== 1 || manifest.status !== "frozen_for_hosted") throw new Error("B0-v4 manifest is not frozen for hosted evaluation");
  if (manifest.productionCandidateSha !== EXPECTED_CANDIDATE || manifest.privateCorpusSha !== EXPECTED_CORPUS || manifest.runtimeKnowledgeVersion !== EXPECTED_RUNTIME) throw new Error("B0-v4 authority pin mismatch");
  const config = manifest.providerConfig;
  if (config.provider !== "Groq" || config.model !== "openai/gpt-oss-120b" || config.temperature !== 0 || config.reasoningEffort !== "low" || config.maxCompletionTokens !== 400 || config.stream !== false || config.directCaseConfidence !== 0.8 || config.tokenBudgetPerMinute !== 6500) throw new Error("B0-v4 provider configuration mismatch");
  const fixtureRaw = readFileSync(resolve(root, manifest.fixture), "utf8");
  const fixtureSha256 = createHash("sha256").update(canonicalText(fixtureRaw), "utf8").digest("hex");
  if (fixtureSha256 !== EXPECTED_FIXTURE_SHA256 || manifest.fixtureSha256 !== EXPECTED_FIXTURE_SHA256) throw new Error("B0-v4 frozen fixture SHA mismatch");
  const fixture = JSON.parse(fixtureRaw);
  if (fixture.schemaVersion !== 1 || fixture.rows.length !== manifest.rows) throw new Error("Invalid frozen B0-v4 fixture");
  ...
  if (runtime.knowledgeVersion !== EXPECTED_RUNTIME) throw new Error("Loaded runtime does not match frozen B0-v4 manifest");
  const resolver = new RuntimeDeterministicSupportResolver();
  const client = new GroqTriageClient({
  ...
  for (let index = 0; index < fixture.rows.length; index += 1) {
    ...
    nextAllowedAt = Date.now() + Math.ceil((estimatedTokens(context.input, config.maxCompletionTokens) / config.tokenBudgetPerMinute) * 60_000);
    ...
    results.push({
      ...
      effectiveAction: triage.decision.nextAction,
      exact,
      restrictedSafe,
      validationErrors: triage.validationErrors,
      latencyMs
    });
    process.stderr.write(`B0-v4 hosted ${index + 1}/${fixture.rows.length}: ${row.id} accepted=${triage.accepted} exact=${exact}\n`);
    if (triage.validationErrors.includes("groq_http_429")) {
  ...
  const summary = {
    ...
    privateCorpusSha: EXPECTED_CORPUS,
    ...
    directCaseConfidence: config.directCaseConfidence,
    benchmarkTokenBudgetPerMinute: config.tokenBudgetPerMinute,
  ...
  writeFileSync(resolve(root, "release-validation/results/b0-v4-hosted-result.json"), `${JSON.stringify(output, null, 2)}\n`, "utf8");
  process.stdout.write(`${JSON.stringify(summary, null, 2)}\n`);
  if (!output.passed) process.exitCode = 1;
}

main().catch((error) => {
  process.stderr.write(`${error instanceof Error ? error.message : "Unknown hosted B0-v4 runner failure"}\n`);
  process.exitCode = 1;
});
